...
# --- Persistent log collector using Streamlit session state ---
...
builtins.print = lambda *args, **kwargs: log_message(
    " ".join(str(a) for a in args), "PRINT"
)


# Streamlit log handler for logging module
class StreamlitLogHandler(logging.Handler):
    def emit(self, record):
        log_message(self.format(record), record.levelname)
...
# --- Main layout: left = app, right = log window ---
...
with main_col:
        ...
        if st.button("Compare APIs", key="comparison_plot_btn"):
            try:
                ...
                if combined_draw:
                    ...
                    st.info(
                        f"VisualCrossing parameters: {', '.join(selected_visualcrossing_parameters)}"
                    )
                    st.info(
    ...
    # Meteomatics Plots Page
        ...
        if not meteomatics_available:
            st.error(
                "Meteomatics credentials not configured. Please check your configuration."
            )
            st.stop()
        ...
        with col1:
            start_date = st.date_input(
                "Start date (UTC)",
                value=aligned_now.date(),
                key="meteomatics_start_date",
            )
            start_time = st.time_input(
                "Start time (UTC)",
                value=aligned_now.time(),
                key="meteomatics_start_time",
            )
        with col2:
            end_date = st.date_input(
        ...
        if st.button("Plot Meteomatics Forecast", key="meteomatics_plot_btn"):
            try:
                ...
                if meteomatics_draw:
                    ...
                    if len(locations) == 1:
                        st.info(
                            f"Plotting Meteomatics forecast for location: ({lat:.4f}, {lon:.4f})"
                        )
                    else:
                        st.info(
                            f"Plotting Meteomatics forecast for {len(locations)} locations"
                        )
                    ...
                    if len(locations) == 1:
                        ...
                        meteomatics_draw.plot_comparison(
                            ...
                            or [
                                "t_2m:C"
                            ],  # Default to temperature if no parameters selected
                            start_datetime=start_dt,
                            end_datetime=end_dt,
                            model=model,
                            interval=interval,
                    ...
                    st.error(
                        "Unable to initialize Meteomatics API. Please check your configuration."
                    )
